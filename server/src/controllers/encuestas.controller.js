...
export const delete_encuesta = async (req, res) => {
  ...
  const { id_evaluacion, id_asignatura  } = req.params;
  ...
  if (verificacion.rowCount === 0) return res.status(404).json({ ok: false, message: 'Usuario no existe' });
      ...
      AND id_pregunta IN (
        SELECT preguntas.id
        FROM preguntas 
        JOIN encuestas ON encuestas.id_tipo_pregunta = preguntas.id_tipo_pregunta
        WHERE encuestas.id = $2 AND encuestas.id_asignatura = $3
      )
    `, [id, id_evaluacion, id_asignatura]);
